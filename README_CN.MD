...
# 可选：开启 Jet 伪彩色深度图，直接发布到 /camera/depth/image_raw
...
开启 `enable_depth_colorizer` 后，`depth/image_raw` 直接发布 `rgb8` 彩色图（OpenCV Jet）。
关闭时该话题仍发布原始 `16UC1` 深度图，彩色图使用 0--10 m 深度范围进行映射。

...
void printHelp() {
  std::cout
      ...
      << "      [--current_ip <ip>] [--port <port>] [--enable_persistent_ip <true|false>]\\\n"
      ...
      << "                             Persistent IP flag for set-ip "
         "(V2 independent flag, legacy false maps to DHCP).\n"
      ...
      << "\n"
      << "  [DHCP]\n"
      << "    Enable DHCP:  ros2 run orbbec_camera ip_config_tool -- \\\n"
      << "                  dhcp \\\n"
      << "                  --current_ip 192.168.1.10 \\\n"
      << "                  --enable_dhcp true\n"
      << "    Disable DHCP: ros2 run orbbec_camera ip_config_tool -- \\\n"
      << "                  dhcp \\\n"
      << "                  --current_ip 192.168.1.10 \\\n"
      << "                  --enable_dhcp false\n"
      << "  [Set IP]\n"
      << "    Enable persistent IP: ros2 run orbbec_camera ip_config_tool -- \\\n"
      << "                          set_ip \\\n"
      << "                          --current_ip 192.168.1.10 \\\n"
      ...
      << "                          --mask 255.255.255.0 \\\n"
      << "                          --gateway 192.168.1.1\n"
      << "    Disable persistent IP: ros2 run orbbec_camera ip_config_tool -- \\\n"
      << "                           set_ip \\\n"
      << "                           --current_ip 192.168.1.10 \\\n"
...
bool parseArgs(int argc, char **argv, CliArgs &args, std::string &error) {
  ...
  if (args.operation == CliArgs::Operation::SET_IP && args.dhcp_option_set) {
    error = "set_ip only supports persistent IP now. Use subcommand 'dhcp' for --enable_dhcp";
    return false;
  }
  ...
  try {
    ...
    if (args.operation == CliArgs::Operation::SET_IP) {
      ...
      if (v2_supported) {
        ...
        device->getStructuredData(OB_STRUCT_DEVICE_IP_ADDR_CONFIG,
                                  reinterpret_cast<uint8_t *>(&ip_config), &data_size);
        ip_config.dhcp = args.persistent_ip ? 0 : 1;
        uint8_t address[4] = {0};
        uint8_t mask[4] = {0};
        uint8_t gateway[4] = {0};
        if (args.persistent_ip && !parseIpString(args.new_ip, address)) {

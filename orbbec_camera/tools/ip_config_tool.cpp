...
int main(int argc, char **argv) {
  ...
  try {
    ...
    if (enable_force_ip) {
      if (force_ip_mac.empty()) {
        RCLCPP_ERROR(logger, "force_ip_mac is required when enable_force_ip=true.");
        rclcpp::shutdown();
        return 1;
      }

      RCLCPP_INFO(logger, "Applying force-ip to MAC %s ...", force_ip_mac.c_str());
      if (context->forceIp(force_ip_mac.c_str(), ip_config)) {
